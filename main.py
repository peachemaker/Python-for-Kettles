...
def add_label5():
    label5=tk.Label(win, text='                                 РЕШЕНИЕ 17 ЗАДАНИЯ ИЗ ЕГЭ ПО ИНФОРМАТИКЕ НА ЯЗЫКЕ PYTHON\n\n'
                              ...
                              ' последовательности.\n\n'
                              '  f = open("17.txt")                             — открываем файл с данными в переменую f\n'
                              '  c = 0                                              — создаём счётчик пар в которых хотя бы одно число делится на 3\n'
                              '  sm = -20000                                    — создаём переменную в которой будет максимальная сумма из всех пар\n'
                              '  a = [int(i) for i in f]                           — записываем файл в массив, преобразуя строчки в числа\n'
                              '  for i in range(len(a)-1):                    — создаём цикл в диапазоне массива\n'
                              '     if a[i] % 3 == 0 or a[i+1] % 3 == 0:  — задаём условие, проверяя которое будем выполнять действия \n'
                              '         c += 1                                      — прибавляем единицу в счётчик пар \n'
                              '         sm = max(sm, a[i] + a[i+1])         — заносим в переменную максимальное значение из двух аргументов\n'
                              '  print(c,sm)                                       — выводим количество пар и максимальную сумму из таких пар\n\n'
                              '                                          ВАЖНО ПОМНИТЬ!\n\n'
                              ...
                              ' должна делать эта программа .\n\n'
                              '  НАЖМИТЕ ЛЕВОЙ КНОПОКОЙ НА СТРАНИЦУ ЧТОБЫ ПРОРЕШАТЬ ПОХОЖИЕ ЗАДАЧИ С САЙТА "РЕШУ ЕГЭ" \n\n'  ,
                   width=98, height=32, anchor='nw', justify='left',font=('Arial Unicode MS',8,'bold'))
